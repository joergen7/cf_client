...
foreign_function_python() ->
    ...
    E = app(5, var(5, f), []),
    DefLst = [assign(1,
                     r_var(f, t_fn([], t_rcd([{out, t_lst(t_bool())}]))),
                     lam(1,
                         [],
                         {frn, f,
                               t_rcd([{out, t_lst(t_bool())}]),
                               l_python(),
                               <<"blablub">>}))],
    ?assertEqual({ok, {[], DefLst, [E]}}, parse(TokenLst)).
...
foreign_function_r() ->
    ...
    DefLst = [assign(1,
                     r_var(f, t_fn([{x, t_str()}], t_rcd([{out, t_str()}]))),
                     lam(1,
                         [{x, t_str()}],
                         {frn, f,
                               t_rcd([{out, t_str()}]),
...
foreign_function_racket() ->
    ...
    DefLst = [assign(1,
                     r_var(f, t_fn([{x, t_str()}, {y, t_file()}], t_rcd([{out, t_str()}]))),
                     lam(1,
                         [{x, t_str()}, {y, t_file()}],
                         {frn, f,
                               t_rcd([{out, t_str()}]),
