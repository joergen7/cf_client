...
format_error({error, type, {isnil_no_list, Info, {E, T}}}) ->
    io_lib:format("type error ~s: empty predicate operand not a list ~s : ~s",
                  [format_info(Info),
                   format_expr(E),
                   format_type(T)]);
...
format_error({error, type, {tl_no_list, Info, {E, T}}}) ->
    io_lib:format("type error ~s: tail operand is not a list ~s : ~s",
                  [format_info(Info),
                   format_expr(E),
                   format_type(T)]);
