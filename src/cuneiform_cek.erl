...
step({Comm,
      {close, _, {lam, _, _, {ntv, EBody}}, EnvLam},
      _,
      {app_fn, _, XeLst, EnvApp, K},
      _}) ->
...
step({Comm,
      E1,
      _,
      {app_arg, Info, ELam, XeLst1, PropLst1, X1, [], _, K},
      Prop1})
  when Prop1 =/= unknown ->
...
step({Comm,
      E1,
      _,
      {for_arg, Info, TRet, XteLst1, X1, T1, [], EBody, Env, K},
      Prop1})
  when Prop1 =/= unknown ->
...
send(Comm, ESnd, K) ->
    ...
    case lists:keyfind(H, 1, Outbox) =/= false orelse
         sets:is_element(H, Awaybox) orelse
         maps:is_key(H, Inbox) of
        true ->
            {Comm, EFut, #{}, K, unknown};
        false ->
