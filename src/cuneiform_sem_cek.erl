...
try_ascend( {{{stalled, E1}, _}, [{cmp_lhs, Info, E2, Env}|K], Outbox} ) ->
  {{E2, Env}, [{cmp_rhs, Info, {stalled, assert_clean( E1 )}}|K], Outbox};
...
try_ascend( {{{stalled, E1}, _}, [{conj_lhs, Info, E2, Env}|K], Outbox} ) ->
  {{E2, Env}, [{conj_rhs, Info, {stalled, assert_clean( E1 )}}|K], Outbox};
...
try_ascend( {{{stalled, E2}, _},
             [{conj_rhs, _, {true, _}}|K],
             Outbox} ) ->
    {{{stalled, assert_clean( E2 )}, #{}}, K, Outbox};
...
try_ascend( {{{true, _}, _},
             ...
             Outbox} ) ->
  {{{stalled, assert_clean( E1 )}, #{}}, K, Outbox};
...
try_ascend( {{{stalled, E1}, _}, [{disj_lhs, Info, E2, Env}|K], Outbox} ) ->
  {{E2, Env}, [{disj_rhs, Info, {stalled, assert_clean( E1 )}}|K], Outbox};
...
try_ascend( {{{stalled, E1}, _},
   ...
   [{app_arg, Info,
              Lam,
              [{X1, {stalled, assert_clean( E1 )}}|PreBindLst],
              X2,
              PostBindLst,
              Env}|K],
   Outbox};
...
try_ascend( {{{stalled, E1}, _}, [{cons_hd, Info, E2, Env}|K], Outbox} ) ->
  {{E2, Env}, [{cons_tl, Info, {stalled, assert_clean( E1 )}}|K], Outbox};

try_ascend( {{E1, _}, [{cons_hd, Info, E2, Env}|K], Outbox} ) ->
  case is_value( E1 ) of
    false -> error( stuck );
    true  -> {{E2, Env}, [{cons_tl, Info, assert_clean( E1 )}|K], Outbox}
  end;
...
try_ascend( {{{stalled, {cons, ConsInfo, E11, E12}}, _},
  ...
  {{F12, #{}},
   [{append_lhs, Info, assert_clean( E2 ), Env}, {cons_tl, ConsInfo, F11}|K],
   Outbox};

try_ascend( {{{stalled, E1}, _}, [{append_lhs, Info, E2, Env}|K], Outbox} ) ->
  {{E2, Env}, [{append_rhs, Info, {stalled, assert_clean( E1 )}}|K], Outbox};
...
try_ascend( {{{cons, ConsInfo, E11, E12}, _},
  ...
  case is_value( E11 ) andalso is_value( E12 ) of
    ...
    true  ->
      {{E12, #{}},
       [{append_lhs, Info, assert_clean( E2 ), Env}, {cons_tl, ConsInfo, E11}|K],
       Outbox}
  end;
...
try_ascend( {{{stalled, E2}, _}, [{append_rhs, Info, {stalled, E1}}|K], Outbox} ) ->
  {{{stalled, assert_clean( {append, Info, E1, E2} )}, #{}}, K, Outbox};
...
try_ascend( {{{stalled, E1}, _}, [{isnil_op, Info}|K], Outbox} ) ->
    {{{stalled, assert_clean( {isnil, Info, E1} )}, #{}}, K, Outbox};
...
try_ascend( {{{stalled, E1}, _},
             ...
             Outbox} ) ->
  {{E2, Env},
   [{rcd_field, Info, [{X1, {stalled, assert_clean( E1 )}}|PreLst], X2, PostLst, Env}|K],
   Outbox};
...
try_ascend( {{{stalled, {rcd, _, EBindLst}}, _},
  ...
  EX1 =
    case is_value( EX ) of
      true  -> EX;
      false -> {stalled, assert_clean( EX )}
    end,
  {{EX1, #{}}, K, Outbox};
  

try_ascend( {{{stalled, E1}, _}, [{proj_op, Info, X}|K], Outbox} ) ->
  {{{stalled, assert_clean( {proj, Info, X, E1} )}, #{}}, K, Outbox};
...
try_ascend( {{{stalled, E1}, _}, [{fix_op, Info}|K], Outbox} ) ->
    {{{stalled, assert_clean( {fix, Info, E1} )}, #{}}, K, Outbox};
    ...
    {L1, L2} ->
      ...
      EFor1 =
        case is_stalled_typed( L2 ) of
          false ->
            assert_clean( {for, Info, Type, L2, EBody} );
          true ->
...
try_ascend( {{{stalled, E1}, _},
   ...
   [{for_arg, Info,
              Type,
              [{X1, T1, {stalled, assert_clean( E1 )}}|PreLst],
              X2,
              T2,
              PostLst,
              assert_clean( EBody ),
...
try_ascend( {{{stalled, {cons, _, EHd, ETl}}, _},
  ...
  ETl1 =
    case is_value( ETl ) of
      true  -> ETl;
      false -> {stalled, assert_clean( ETl )}
    end,

  EAcc1 = bind_all( Info, [{XAcc, TAcc, assert_clean( EAcc )}, {XArg, TArg, assert_clean( EHd )}], EBody ),

  {{ETl1, #{}},
   [{fold_arg, Info, {XAcc, TAcc, assert_clean( EAcc1 )}, XArg, TArg, assert_clean( EBody ), Env}|K],
   Outbox};

try_ascend( {{{stalled, EArg}, _},
             [{fold_arg, Info, {XAcc, TAcc, EAcc}, XArg, TArg, EBody, _}|K],
             Outbox} ) ->
  {{{stalled, assert_clean( {fold, Info, {XAcc, TAcc, EAcc}, {XArg, TArg, EArg}, EBody} )}, #{}},
   K,
   Outbox};
...
-spec assert_clean( tuple() ) -> e().

assert_clean( E = {var, _, _} )                 -> E;
assert_clean( E = {lam_frn, _, _, _, _, _, _} ) -> E;
assert_clean( E = {fut, _, _, _} )              -> E;
assert_clean( E = {err, _, _, _} )              -> E;
assert_clean( E = {str, _, _} )                 -> E;
assert_clean( E = {file, _, _, _} )             -> E;
assert_clean( E = {true, _} )                   -> E;
assert_clean( E = {false, _} )                  -> E;
