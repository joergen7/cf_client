...
Nonterminals
  arg arg_lst define define_lst expr imp imp_lst imp_f_lst lang ptn
  script type.
...
define     -> assign ptn eq expr semicolon
                              : {'$2', '$4'}.
define     -> def id lparen rparen in lang body
                              : visit_def_frn( '$2', '$6', '$7' ).
...
expr       -> lambda lparen rparen expr
                              : visit_lambda( '$1', [], '$4' ).
expr       -> lambda lparen arg_lst rparen expr
                              : visit_lambda( '$1', '$3', '$5' ).

arg_lst    -> arg             : ['$1'].
arg_lst    -> arg comma arg_lst
                              : ['$1'|'$3'].

arg        -> id colon type   : visit_arg( '$1', '$3' ).
...
-spec visit_arg( {id, _, X :: string()}, T :: t() ) -> lam_ntv_arg().

visit_arg( {id, _, X}, T ) ->
  cuneiform_lang:lam_ntv_arg( list_to_atom( X ), X, T ).

-spec visit_def_frn( Id, Lang, Body ) -> {r(), e()}
when Id   :: {id, pos_integer(), string()},
     Lang :: l(),
     Body :: {body, _, string()}.

visit_def_frn( {id, Line, SName}, Lang, {body, _, SBody} ) ->
  {cuneiform_lang:r_var(
     Line,
     list_to_atom( SName ),
     cuneiform_lang:t_fn( frn,
                          [],
                          cuneiform_lang:t_str() ) ),
   cuneiform_lang:lam_frn( Line, SName, [], Lang, SBody )}.